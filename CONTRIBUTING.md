# Contribution Guidelines

## Pull Requests

- Appease the linter
- Bump the version
- Adjust the Readme according to your changes (if applicable)
- No extra dependencies unless absolutely necessary
- If you're fixing an issue, open an issue as well or link existing one

## Issues

- Use the issue templates
- When requesting a feature, check it hasn't been [rejected](https://github.com/simoabid/cineflix-cli/issues/523) previously
- Provide screenshot if applicable

## How else can I help?

- Ask the devoloper at his [Portfolio](https://abidev.dev/)
- Take part in troubleshooting and testing
- Star the repo
- Follow the maintainers
